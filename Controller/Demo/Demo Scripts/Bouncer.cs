    ...
    public class Bouncer : MonoBehaviour {
        ...
        private void OnTriggerEnter2D(Collider2D collision) {
            if (collision.TryGetComponent(out IPlayerController controller)) {
                ...
                controller.ApplyVelocity(-incomingSpeedNormal, UnitForce.Burst); // cancel current vertical speed for more consistent heights
                controller.SetVelocity(transform.up * _bounceForce, UnitForce.Decay);
            }
        }
    }
}